...
(function() {
    ...
    try {
      ...
      if (settings) {
        ...
        if (settings.chatWidgetText !== undefined) {
          // Пустая подпись в админке намеренно отключает вращающийся текст.
          chatRingText = String(settings.chatWidgetText || '').trim().slice(0, 30);
        }
      }
    } catch (e) {
  ...
  function syncToggleIcon(isOpen) {
    ...
    if (isOpen) {
      ...
      var ringHtml = buildChatRingLabel(chatRingText);
      els.toggle.innerHTML = ringHtml + '<span class="chat-glyph" aria-hidden="true"></span>';
    }
